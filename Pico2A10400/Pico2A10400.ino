...
void __time_critical_func(emulate_normala78_hsc()) {
  ...
  //
  // BOARD DIFFERENCE, and it is NOT a copy of PicoA10400.ino - the same note
  // emulate_normala78() carries a few lines up. Here BUS_PIN_MASK covers only
  // A0-A14 and A15 is gpio 26, so the full 16-bit address has to be reassembled
  // before it can be compared against a $0000-$FFFF boundary, and every
  // end-of-cycle test compares BUS15_PIN_MASK. 0.52-0.54 shipped the PicoA10400
  // loop here verbatim: `addr` then topped out at $7FFF, so the ROM window never
  // matched for a 16K or 32K cart and $9000-$97FF aliased into the NVRAM window.
  ...
  while (1) {
    ...
    if (addr >= lo) {
      ...
      // must not put us on the bus against the CPU.
      if (gpio_get_all() & RW_PIN_MASK) {
        sio_hw->gpio_out = (uint32_t)bios[addr & 0x0FFF] << D0_PIN;
        SET_DATA_MODE_OUT;
        for (uint32_t k = 0; k < 256; k++)
          if ((gpio_get_all()&BUS15_PIN_MASK) != rawaddr) break;
        SET_DATA_MODE_IN;
        hsc_bios_reads++;
      }
      ...
      if (gpio_get_all() & RW_PIN_MASK) {
        ...
        for (uint32_t k = 0; k < 256; k++) {
          ...
          if ((h1 & BUS15_PIN_MASK) != rawaddr) break;
          if (h1 & RW_PIN_MASK) continue;
          const uint32_t h2 = gpio_get_all();
          if ((h2 & BUS15_PIN_MASK) != rawaddr) break;
      ...
      if (wr) {
        // End-of-cycle capture, bounded at 64 like every other write path on
        // this board - and comparing BUS15_PIN_MASK, so A15 moving ends the
        // cycle instead of being invisible.
        uint32_t last = gpio_get_all(), cur;
        for (uint32_t g = 0; g < 64; g++) {
          cur = gpio_get_all();
