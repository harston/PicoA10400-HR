...
void AtariMenu(int tipo) { // 1=start,2=next page, 3=prev page, 4=dir up
  ...
  switch (tipo) {
    case 1: { // root dir
      ...
      // pass 3: render sorted filelist into menu_ram. The high bit of a character is never
      // drawn (the glyph renderer masks with AND #$7F), so it carries per-entry colour flags:
      // char[0] = directory, char[1] = oversized ROM (drawn red). See menu_rom_analysis/.
