// pokey.h - minimal POKEY audio synthesis for PicoA10400
//
// PURPOSE: answer one question on real hardware - "do POKEY games produce sound
// through cartridge pin 18?". This is deliberately NOT a faithful POKEY. It is the
// smallest thing that should make Ballblazer's music recognisable.
//
...
//   drives the console's audio input directly with PWM. Expect it to be loud and to
//   carry the PWM carrier; that is understood and accepted for a first test. If the
//   picture picks up interference, an RC low-pass is the fix (1k + 10nF), not a code
//   change.
//
// MODEL: per-output-sample, not per-POKEY-cycle. The reference implementation
// (ORIG/JS7800/src/js/prosystem/Pokey.js, ported from ProSystem) ticks every channel
// at 1.79MHz and samples the polynomial counters on each divider borrow. Doing that
// here would mean ~1.79M iterations/second on core 0, which on an RP2040 would both
// be tight AND fight core 1 for SRAM on every access - exactly the kind of
// contention that took several builds to get rid of. Instead each channel keeps a
// phase accumulator advanced once per output sample. Pitch and volume come out
// right, which is what carries a melody; noise texture is approximate.
//
// KNOWN SIMPLIFICATIONS (all deliberate, all cheap to lift later):
//   - high-pass filter modes (AUDCTL bits 1,2) ignored
//   - 9-bit poly (AUDCTL bit 7) treated as the 17-bit poly
//   - the 5-bit poly gate is applied, but sampled per channel period rather than
//     from a globally free-running counter
//   - no POKEY reads: RANDOM etc. are not driven, the bus stays open there
//   - no serial port, no pots, no IRQs, no timers
